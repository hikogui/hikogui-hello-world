...
To build:
 1. Select `MSVC-x64-Debug (vcpkg)` from the project `Configuration` pull down menu.
...
Other configurations that are possible to build:
 - `x64-MSVC-Debug (vcpkg)` Make a debug build with the current release version of HikoGUI.
 - `x64-MSVC-Debug (vcpkg:main)` Make a debug build with the main branch of HikoGUI.
 - `x64-MSVC-Release (vcpkg)` Make a fully optimized release build with the current release version of HikoGUI.
 - `x64-MSVC-Release (vcpkg:main)` Make a fully optimized release build with the main branch of HikoGUI.
